...
int main(int argc, char **argv)
{
    ...
    double state[NR_OF_STATES] = {0, 0, 0, 2};
    double position[3];
    ...
    for (int i = 1; i < 100; i++)
    {
        ...
        predict(get_target_speed(target_yaw), x_calc, y_calc, v, state[0], state[1], state[2], target_yaw, controls, state);
