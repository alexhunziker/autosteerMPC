...
#include "mpc_gps_unconstr_ss.cpp"
...
extern "C"
{
    ...
    void predict(
        ...
        double yaw_target, double *controls, double *predicted_state)
    {

        matrix<double, NR_OF_STATES, 1> current_state;
        current_state = yaw, yaw_rate, schwimm_angle, speed;
        ...
        target_state = yaw_target, 0, 0, speed_target;
        if (DEBUG)
        {
...
mpc<NR_OF_STATES, NR_OF_CONTROLS, PREDICTION_HORIZON> initialize_mpc_object(int i)
{
    ...
    R = 0.3, 0.2, 0.2; // Dlib does not allwo to punish rates
