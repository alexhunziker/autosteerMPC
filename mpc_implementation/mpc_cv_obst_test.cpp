...
int main(int argc, char **argv)
{
    ...
    double state[NR_OF_STATES] = {0, 0, 0, 0, 0, 2, 3};
    double position[3];

    for (int i = 1; i < 100; i++)
    {
        double v = sanitize_velocity(state[5]);
        calc_position(state[1], state[2], v);
        predict(get_target_speed(), x_calc, y_calc, v, get_y_offset(v, TARGET_YAW, state[2]), state[1], state[2], state[3], TARGET_YAW, get_collision_distance(v), controls, state);
