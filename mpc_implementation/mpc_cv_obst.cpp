...
extern "C"
{
    void initialize_mpc_objects()
    {
        ...
        if (DEBUG)
            printf("DEBUG: MPC Objects set up for CV Obstacle.\n");
    }
    ...
    void predict(
        ...
        double yaw_rate_target, double collision_time, double *controls, double *predicted_state)
    {

        matrix<double, NR_OF_STATES, 1> current_state;
        current_state = y_offset, yaw, yaw_rate, schwimm_angle, yaw_rate_target, speed, collision_time;
        ...
        target_state = 0, 0, yaw_rate_target, 0, yaw_rate_target, speed_target, TARGET_TIME_TO_COLLISION;
        if (DEBUG)
...
mpc<NR_OF_STATES, NR_OF_CONTROLS, PREDICTION_HORIZON> get_best_mpc_object(int speed)
{
    ...
    if (DEBUG)
        cout << "DEBUG: MPC CV obst used with parameters idx " << i << "\n";
